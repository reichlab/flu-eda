...
<<PlotNationalRegionalNumILI, fig.width = 14, fig.height = 10.25>>=
flu <- flu %>%
...
<<BasicBackfillLM>>=
lm_fit <- remlscore(
  y = flu$diff_log_curr_final_num_ili,
  X = cbind(rep(1, nrow(flu)), flu$lag, flu$lag^2),
  Z = as.matrix(flu$lag)
)
...
ribbons_df <-
  ...
  mutate(
    pred_mean = cbind(rep(1, 51), lag, lag^2) %*% matrix(lm_fit$beta),
    pred_var = (rep(1, 51) * lm_fit$se.beta[1])^2 + (lag * lm_fit$se.beta[2])^2 + (lag^2 * lm_fit$se.beta[3])^2 + exp(lag * lm_fit$gamma[1]),
    pred_sd = sqrt(pred_var),
    pred_95_lb = pred_mean - 2 * pred_sd,
    pred_95_ub = pred_mean + 2 * pred_sd,
    pred_68_lb = pred_mean - 1 * pred_sd,
    pred_68_ub = pred_mean + 1 * pred_sd
  )
...
ggplot() +
  geom_ribbon(aes(x = lag, ymin = pred_95_lb, ymax = pred_95_ub),
    alpha = 0.2,
    data = ribbons_df) +
  geom_ribbon(aes(x = lag, ymin = pred_68_lb, ymax = pred_68_ub),
    alpha = 0.2,
    data = ribbons_df) +
  geom_line(aes(x = lag, y = pred_mean),
    data = ribbons_df) +
  geom_point(aes(x = lag, y = diff_log_curr_final_num_ili),
    alpha = 0.2,
    data = flu) +
  theme_bw()
@
...
bs_basis_lag_for_plot <- bs(x = 1:51,
  df = bs_spline_df,
  intercept = TRUE)
...
ribbons_df <-
  ...
  mutate(
    pred_mean = bs_basis_lag_for_plot %*% matrix(lm_fit$beta),
    pred_var = apply(
      bs_basis_lag_for_plot^2 %*% matrix(lm_fit$se.beta)^2 +
        exp(bs_basis_lag_for_plot %*% matrix(lm_fit$gamma)),
      1,
      sum),
    pred_sd = sqrt(pred_var),
